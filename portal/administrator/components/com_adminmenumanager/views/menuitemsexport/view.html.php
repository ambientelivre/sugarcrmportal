...
class adminmenumanagerViewMenuitemsexport extends JViewLegacy{		
	...
	public function display($tpl = null){	
		...
		//toolbar
		JToolBarHelper::custom( 'back_to_menuitems', 'back.png', 'back.png', JText::_('JTOOLBAR_BACK'), false, false);	//does not show icon in Joomla 3.
	...
	function get_csv_string(){
		...
		if (count($cid)){			
			...
			for($i=0; $i < count( $rows ); $i++) {
				...
				$return .= $delimiter;
				$return .= $quote.$row->title.$quote;
				$return .= $delimiter;
				$return .= $quote.$row->icon.$quote;
				$return .= $delimiter;
				$return .= $quote.$row->url.$quote;
				$return .= $delimiter;	
