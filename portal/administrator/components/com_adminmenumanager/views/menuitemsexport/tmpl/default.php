...
Joomla.submitbutton = function(task){	
	...
	if(task=='download_csv'){					
		submitform('download_csv');
	}	
	if(task=='download_txt'){					
		submitform('download_txt');
	}
...
<form action="" method="post" name="adminForm">
	<div class="width-100 fltlft">	
		...
		<fieldset class="adminform">	
			<legend class="amm_legend"><?php echo JText::_('COM_MENUS_SUBMENU_ITEMS').' '.$this->controller->amm_strtolower(JText::_('JTOOLBAR_EXPORT')); ?></legend>	
			<table class="adminlist tabletop amm_table">							
				...
				<tr>
					<td>						
						<button onclick="submitbutton('download_csv');"><?php echo JText::_('COM_ADMINMENUMANAGER_DOWNLOAD').' '.JText::_('COM_ADMINMENUMANAGER_AS').' .csv '.$this->controller->amm_strtolower(JText::_('JLIB_FORM_VALUE_CACHE_FILE')); ?></button>	
						<?php
				...
				<tr>
					<td>
						<button onclick="submitbutton('download_txt');"><?php echo JText::_('COM_ADMINMENUMANAGER_DOWNLOAD').' '.JText::_('COM_ADMINMENUMANAGER_AS').' .txt '.$this->controller->amm_strtolower(JText::_('JLIB_FORM_VALUE_CACHE_FILE')); ?></button>	
						<?php
						if($this->controller->get_version_type()=='free'){
							echo '<span class="red">';
							echo JText::_('COM_ADMINMENUMANAGER_NOT_IN_FREE_VERSION');
