...
class adminmenumanagerController extends JControllerLegacy{
	...
	function display_header(){					
		...
		if(JRequest::getVar('layout')!='csv'){		
			...
			echo '<link href="components/com_adminmenumanager/css/adminmenumanager6.css" rel="stylesheet" type="text/css" />'."\n";
		}
		
		/*		
		echo '<div id="amm_header_messages">';		
		echo '</div>';	
		*/	
	}						
			...
			if($latest_version_message==''){
				...
				if($file_object == TRUE){
					...
					if($this->version!=$version){
						...
						$latest_version_message .= ' <span class="am_green">'.JText::_('COM_ADMINMENUMANAGER_IS_LATEST_VERSION').'</span>';
					}
					fclose($file_object);
				}				
	...
	function ajax_version_checker(){
		...
		if($file_object == TRUE){
			...
			if($this->version!=$version){
				...
				$message .= '<div><span class="am_green">'.JText::_('COM_ADMINMENUMANAGER_IS_LATEST_VERSION').'</span>.</div>';
			}
			fclose($file_object);
		}
