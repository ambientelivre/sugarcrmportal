...
';				
	...
	function echo_header(){	
		...
		//display message that FUA will not be for Joomla 3
		if(!$this->fua_config['hide_joomla3_message']){
			echo '<div style="text-align: left;" class="fua_red">'.JText::_('COM_FRONTENDUSERACCESS_NOT_FOR_JOOMLA_THREE').' <a href="http://www.pages-and-items.com/extensions/access-manager" target="_blank">Access-Manager</a>.';
			echo '&nbsp; <a href="index.php?option=com_frontenduseraccess&task=do_not_show_joomla3_message&returnview='.$this->view.'" title="'.htmlspecialchars(JText::_('COM_FRONTENDUSERACCESS_DONT_SHOW_MESSAGE_AGAIN')).'"><img src="components/com_frontenduseraccess/images/close.png" /></a>';
			echo '<br/><br/></div>';	
		}
...
menu_allow_registration='.JRequest::getVar('menu_allow_registration', '', 'post').'
menu_allow_searchresults='.JRequest::getVar('menu_allow_searchresults', '', 'post').'
hide_joomla3_message='.JRequest::getVar('hide_joomla3_message', '', 'post').'
';
	...
	function display_footer(){	
		...
		if($this->fua_config['version_checker']){
			...
			if($latest_version_message==''){
				...
				$url = 'http://www.pages-and-items.com/latest_version.php?extension=frontenduseraccess';		
				...
				if($file_object == TRUE){
					...
					if($this->version!=$version){
						...
						}elseif($this->fua_version_type=='trial'){
						...
						if($this->fua_version_type!='pro'){
							$latest_version_message .= ' <a href="index.php?option=com_installer&view=update">'.$this->fua_strtolower(JText::_('JLIB_INSTALLER_UPDATE')).'</a>';
						}
					}else{
		...
		echo $this->fua_strtolower(JText::_('COM_FRONTENDUSERACCESS_REVIEW_B')); 
		echo '</td>';
		echo '<td>';
		if($this->fua_version_type=='pro'){
			$url_jed = '6874';
	...
	function reverse_access_warning($which){
		...
		if($this->fua_config[$which]){				
			...
			echo '<img src="components/com_frontenduseraccess/images/icon-16-notice.png" class="warning_img" alt="be carefull" />'.JText::_('COM_FRONTENDUSERACCESS_REVERSE_ACCESS_WARNING');
		}else{
	...
	function ajax_version_checker(){
		...
		if($file_object == TRUE){
			...
			if($this->version!=$version){
				...
				$message .= '<div>';
				$message .= '<a href="'.$download_url.'" target="_blank">'.JText::_('COM_FRONTENDUSERACCESS_DOWNLOAD').'</a>';
				if($this->fua_version_type!='pro'){
					$message .= '<br /><a href="index.php?option=com_installer&view=update">'.$this->fua_strtolower(JText::_('JLIB_INSTALLER_UPDATE')).'</a>';
				}
				$message .= '</div>';				
	...
	function do_not_show_joomla3_message(){
		...
		//change config
		$config = str_replace('hide_joomla3_message=','hide_joomla3_message=true',$raw);	

		//update config
		$query = $db->getQuery(true);		
		$query->update('#__fua_config');
		$query->set('config='.$db->q($config));		
		$query->where('id='.$db->q('fua'));
		$db->setQuery($query);
		$db->query();
		
		$app->redirect('index.php?option=com_frontenduseraccess&view='.$returnview);
	}
