...
Joomla.submitbutton = function(task){		
	...
	if (task=='weblinks_apply'){				
		submitform('weblinks_save');		
	}
	if (task=='weblinks_save'){	
...
<form name="adminForm" id="adminForm" method="post" action="<?php echo JRoute::_('index.php?option=com_accessmanager&view=weblinks'); ?>">	
	...
	<div id="j-main-container"<?php echo empty($this->sidebar) ? '' : ' class="span10"'; ?>>
		...
		<table id="am_subheader">
			<tr>
				<td>	
					<p>
						<span class="am_pagetitle"><?php echo JText::_('COM_WEBLINKS'); ?></span>
						<?php echo JText::_('COM_ACCESSMANAGER_FRONTEND').' '.$this->helper->am_strtolower(JText::_('COM_WEBLINKS').' '.JText::_('JFIELD_ACCESS_LABEL')); ?>.
					</p>
					<?php	
							
					//message if item access is not activated		
					if($this->controller->am_config['weblink_active']==false){				
						echo '<p class="am_red">'.JText::_('JNO').' '.JText::_('COM_WEBLINKS').' '.$this->helper->am_strtolower(JText::_('JFIELD_ACCESS_LABEL')).' '.$this->helper->am_strtolower(JText::_('JENABLED')).'. '.JText::_('COM_ACCESSMANAGER_ACTIVATE_IN_PANEL_UNDERNEATH').'.</p>';
					}					
					...
					echo JHtml::_('sliders.start','config-weblink-access', array('useCookie'=>1, 'show'=>-1, 'display'=>-1, 'startOffset'=>-1));
					...
					<div class="border_bottom">
						<table class="adminlist am_table">
							<tr>		
								<td width="300">
									<?php echo $this->helper->am_strtolower(JText::_('COM_WEBLINKS')).' '.$this->helper->am_strtolower(JText::_('JFIELD_ACCESS_LABEL')).' '.$this->helper->am_strtolower(JText::_('JENABLED')); ?>
								</td>
								<td>
									<input type="checkbox" class="checkbox" name="weblink_active" value="true" <?php if($this->controller->am_config['weblink_active']){echo 'checked="checked"';} ?> />
								</td>
								<td>					
									<?php 
										echo JText::_('COM_ACCESSMANAGER_FRONTEND').' ';
										echo $this->helper->am_strtolower(JText::_('COM_WEBLINKS_COMPONENT_LABEL')).' ';
							...
							<tr>		
								...
								<td>
									<input type="checkbox" class="checkbox" name="weblink_reverse_access" value="true" <?php if($this->controller->am_config['weblink_reverse_access']){echo 'checked="checked"';} ?> />
								</td>
								<td>
									<?php 
							...
							<tr>		
								...
								<td>
									<label style="white-space: nowrap;"><input type="radio" name="weblink_multigroup_access_requirement" value="one_group" class="radio" <?php if($this->controller->am_config['weblink_multigroup_access_requirement']=='one_group'){echo 'checked="checked"';} ?> /><?php 
									if($this->controller->am_config['based_on']=='level'){
									...
									?></label><br />					
									<label style="white-space: nowrap;"><input type="radio" name="weblink_multigroup_access_requirement" value="every_group" class="radio" <?php if($this->controller->am_config['weblink_multigroup_access_requirement']=='every_group'){echo 'checked="checked"';} ?> /><?php 					
									if($this->controller->am_config['based_on']=='level'){
							...
							<tr>		
								...
								<td>
									<input type="checkbox" class="checkbox" name="weblink_superadmins" value="true" <?php if($this->controller->am_config['weblink_superadmins']){echo 'checked="checked"';} ?> />
								</td>
								<td>
							...
							<tr>		
								...
								<td colspan="2">
									<?php	
										//weblink_default_access		
										echo $this->helper->display_multigrouplevel_select_config('weblink', $this->controller->am_config, 1);				
									?>					
								
									<?php 
										echo JText::_('COM_ACCESSMANAGER_DEFAULT_ACCESS_INFO').' '; 
										echo $this->controller->am_strtolower(JText::_('COM_WEBLINKS_COMPONENT_LABEL')); 
									?>.					
								</td>
							</tr>
						</table>
					...
					<table style="width: 100%;">
						<tr>
							<td>
								...
								$this->controller->reverse_access_warning('weblink_reverse_access');	
		...
		<table class="adminlist table table-striped" style="width: 100%;">
			<tr>		
				...
				<th align="left" class="nowrap">				
					<?php 
					$label = ucfirst(JText::_('JGLOBAL_TITLE')).' '; 			
			...
			<?php
				...
				foreach($this->items as $item){	
					...
					foreach($this->am_grouplevels as $am_accesslevel){
						$checked = '';
						$checked_hidden = '';
						if (in_array($item->id.'__'.$am_accesslevel->id, $this->access_weblinks)) {
