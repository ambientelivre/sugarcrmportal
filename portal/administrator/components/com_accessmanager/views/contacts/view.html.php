...
class accessmanagerViewContacts extends JViewLegacy{
	...
	function display($tpl = null){
		...
		$helper->clean_access_table('contact', 'contact_details', 'published');			
		
		//toolbar		
		JToolBarHelper::apply('contacts_apply');
		JToolBarHelper::save('contacts_save');
		...
		//get access from db
		$helper = new accessmanagerHelper();
		$access_contacts = $helper->get_access_rights('contact', $this->controller->am_config['based_on']);	
		$this->assignRef('access_contacts', $access_contacts);		
		
		//set header		
		JToolBarHelper::title('Access Manager :: '.$this->helper->am_strtolower(JText::_('COM_CONTACT_SUBMENU_CONTACTS')), 'am_icon');	
	...
	function add_sidebar($helper){
		...
		JHtmlSidebar::setAction('index.php?option=com_accessmanager&view=contacts');	
	...
	protected function getSortFields(){
		...
		return array(
			'c.name' => JText::_('COM_ACCESSMANAGER_NAME'),
