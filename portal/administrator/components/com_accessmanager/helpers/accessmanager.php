...
class accessmanagerHelper{	
	...
	function add_submenu(){	
		...
		JHtmlSidebar::addEntry(
			JText::_('COM_ACCESSMANAGER_ACCESS_VIEWING'),
			'index.php?option=com_accessmanager&view=accessvievving',
			$vName == 'accessvievving' || $vName == 'articles' || $vName == 'categories' || $vName == 'modules' || $vName == 'components' || $vName == 'menuaccess' || $vName == 'parts' || $vName == 'part' || $vName == 'adminmenumanager' || $vName == 'modulesadmin'
		);
		JHtmlSidebar::addEntry(
			JText::_('COM_ACCESSMANAGER_ACCESS_EDITTING'),
			'index.php?option=com_accessmanager&view=accessedit',
			$vName == 'accessedit' || $vName == 'modulesbackend' || $vName == 'componentsbackend' || $vName == 'menuitemsbackend' || $vName == 'pluginsbackend'
		);
		JHtmlSidebar::addEntry(
			JText::_('COM_ACCESSMANAGER_TOOLS'),
			'index.php?option=com_accessmanager&view=tools',
	...
	function clean_access_table($type, $table, $column_enabled){
		...
		$items_array = $database->loadColumn();
	...
	function display_multigrouplevel_select_config($type, $am_config, $include_superadmin = 0, $backend_only = 0, $only_groups = 0){
		...
		$html = '<select name="'.$type.'_default_access[]" id="'.$type.'_default_access" multiple="multiple" size="'.count($accesslevels).'" class="chzn-done">';
		...
		$html .= "JNC_jQuery(".$type."_default_access).chosen('destroy');";	
		$html .= "});";
		$html .= '</script>';	
	...
	function give_authors_group_access($item, $type){
		...
		//insert right for each of the groups
	...
	function clear_article_cache(){
		$this->clear_cache('com_content');	
		$this->clear_cache('mod_articles_archive');
		$this->clear_cache('mod_articles_categories');
		$this->clear_cache('mod_articles_category');
		$this->clear_cache('mod_articles_latest');
		$this->clear_cache('mod_articles_news');
		$this->clear_cache('mod_articles_popular');	
	}	
